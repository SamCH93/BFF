...
To understand behaviour of the MEE with increasing sample size, we may look at
an approximation of the Bayes factor. Suppose that the data
...
maximizer under the alternative hypothesis. Denote by $n V_{0}$ and $n V_{1}$
the modal dispersion matrices (minus the inverse of the matrix of second-order
partial derivatives of the log likelihood evaluated at the corresponding
maximizer). Applying a Laplace approximation to the logarithm of the BFF
\cite[equation 7.27]{OHagan2004} gives then
% \begin{align*}
...
the parameter estimates lowers the evidence for the corresponding hypothesis.
In sum, finding the MEE  corresponds to
...
This shows that REML estimates can be motivated from a Bayesian evidence
...
over an alternative hypothesis, while the MLE is defined without reference to
alternatives.
...
association between 14 exposure variables and the neonatal death. Among the 2992
...
independent
$p(\log \mathrm{OR}_{i} \mid H_{1}) = \Nor(\log \mathrm{OR}_{i} \mid 0, 1/2)$
priors were assigned to each coefficient under the alternative. This prior
represents an alternative postulating that the median odds ratio is $1$ and that
$95\%$ of odds ratios are in between $1/4$ and $4$ \citep{Greenland2006}. For
the analysis of each coefficient, all other coefficients were considered as
nuisance parameters with the same priors assigned to them under the null as
under the alternative.

BFFs were computed in three ways: i) By first computing the marginal posterior
distribution for each coefficient from 1'000'000 Markov chain Monte Carlo (MCMC)
samples (solid black line) as implemented in Stan \citep{Carpenter2017} and then
computing the BFF via the Savage-Dickey density ratio. ii) Computing the
marginal posterior with intergrated nested Laplace approximation (dashed blue
line) as implemented in INLA \citep{Rue2009} and then computing the BFF via the
Savage-Dickey density. iii) The logistic model was first estimated with maximum
likelihood, and each estimated coefficient and its standard error were then used
for a univariate normal analysis as explained in Section~\ref{sec:normalmean}
ignoring the nuisance parameters. The MCMC analysis took the longest among the
three (several minutes), folled by the INLA analysis (several seconds), and the
univariate analysis (almost instantly). Moreover, we can see that the MCMC and
INLA methods show inaccuracies in the tails of the BFF. The univariate normal
analysis aligns well with the more complex MCMC and INLA analyses in most cases.
...
when a BFF is computed via the Savage-Dickey density from a posterior
