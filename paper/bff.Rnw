...
and Bayes factors (as shown in the right plot in Figure~\ref{fig:pvalfun}),
establishing a duality between hypothesis testing and parameter estimation. Our
...
This provides data analysts with a unified framework for statistical inference
centred around the Bayes factor. Related ideas have recently been proposed in
the physics community under the name of `Bayes factor surface'
\citep{Fowlie2024} and `\textit{K} ratio` \citep{Afzal2023}.
...
the Savage-Dickey density representation applies to the BFF.
...
correspond to the maximum likelihood estimates, while the MEE from the MCMC and
INLA analyses correspond to the integrated maximum likelihood estimates.

The MCMC analysis took the longest of the three (several minutes to run),
...
increase monotonically but end abruptly around OR = 30 because no larger MCMC
samples were observed or because the INLA algorithm returned a posterior density
of zero. However, the shapes of the BFFs suggest that their maxima lie outside
the range shown, possibly close to the maxima of the univariate normal BFF.
Because of these computational issues, the `Hydramnios' BFF should be
