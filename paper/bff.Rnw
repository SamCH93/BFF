...
Bayes factor \citep{Jeffreys1939, Good1958, Kass1995, Held2018<}, the data-based
...
evidence levels can also be used. For example, based on the conventions from
...
$\sigma = \Sexpr{round(se, 3)}$ estimated with Cox regression). To obtain a
Bayes factor for contrasting $H_{0} \colon \theta = \theta_{0}$ against
...
based on eight previous trials, which could be used to set the prior mean and
variance to $m = \Sexpr{round(m, 2)}$ and variance
$v = \Sexpr{round(s, 3)}^{2}$, see Figure~\ref{fig:normal} for the resulting BFF
(orange). In this case, the MEE is given by $\thatME =y = \Sexpr{round(y, 2)}$
with the support interval centered around it. Due to the apparent conflict
between the observed data and the specified prior under the alternative, the
$k = 1$ support interval spans an wide range from $\Sexpr{round(sis[1,1], 2)}$
to $\Sexpr{round(sis[1,2], 2)}$, indicating that very beneficial up to slightly
harmful treatment effects are supported by the data over the alternative.
...
e.g.,][]{Dawid2011}. Since the BFF is nothing else than the Bayes factor
evaluated for various null hypotheses, this consistency property carries over to
the BFF. That is, as more data are generated from the model with true parameter
$\theta_{*}$, the BFF at $\theta_{0} = \theta_{*}$ will go to infinity, while
...
To understand the asymptotic behaviour of the MEE, we may look at an
approximation of the Bayes factor. Suppose that the data
$Y = \{Y_{1}, Y_{2}, \dots, Y_{n}\}$ are independent and identically distributed
and denote by $\hat{\psi}_{0}$ the maximizer of the log likelihood of the data
under the null and by $(\hat{\theta}_{1}$, $\hat{\psi}_{1})$ the maximizer under
the alternative hypothesis. Denote by $n V_{0}$ and $n V_{1}$ the modal
dispersion matrices (minus the inverse of the matrix of second-order partial
derivatives of the log likelihood evaluated at the corresponding maximizer).
Applying a Laplace approximation to the logarithm of the BFF \cite[equation
7.27]{OHagan2004} gives then
% \begin{align*}
...
% (fourth term).
Equation~\eqref{eq:bfapprox} has much intuitive value: as $\theta_{0}$ becomes
...
lowers the evidence for the corresponding hypothesis.
...
prior for the nuisance parameters and the modal dispersion. It is clear that for
...
\end{align}
That is, the posterior can be obtained by multiplying the BFF with the prior. It
is, however, important to emphasize that BFFs based on priors under the
...
normally distributed around the maximum likelihood estimate with a covariance
matrix equal to the observed information matrix \citep[chapter
5.3]{Bernardo2000}, we can conclude that whenever the BFF has the Savage-Dickey
density ratio representation~\eqref{eq:posterior}, asymptotically the BFF is the
asymptotic posterior normal density divided by the prior density, both evaluated
at $\theta_{0}$. Hence, in contrast to the posterior distribution, the influence
of the prior on the BFF does not `wash` out as more data are collected.
...
Clearly this estimate equivalent to the MEE whenever the BFF and relative belief
ratio coincide. \citet{Evans1997} also defined a $\gamma$ \emph{relative
...
we can see the result reported by \citet{Bartos2023}: There is extreme evidence
...
the study from \citet{Neutra1978}, previously reanalyzed by
\citet{Greenland2007, Sullivan2012}. This epidemiological study investigated the
association between 14 exposure variables and the risk for neonatal death. Among
the 2992 births only 17 neonatal deaths occurred, which leads to challenges in
conducting inferences with so many exposure variables.


\begin{figure}[!htb]
<< "regression-example-mcmc", cache = TRUE >>=
## reanalyze the data from Sullivan and Greenland (2008)
dat <- read.csv(file = "../sullivangreenland2013.csv")
...
bglm1 <- brm(death ~ ., data = dat,
             ...
             iter = 1000000, warmup = 1000, cores = 12,
             family = "bernoulli")
...
@
<< "regression-analysis", fig.height = 8.8, fig.width = 8, cache = TRUE >>=
...
logORseq <- seq(log(1/10), log(100), length.out = 100)
ORseq <- exp(logORseq)
bfbks <- c(1/1000, 1/100, 1/10, 1, 10, 100, 1000)
bflabs <- c("1/1000", "1/100", "1/10", "1", "10", "100", "1000")
...
for (i in seq(1, length(coefs))) {
    ...
    lines(ORseq, bffglm, lty = 3, col = 2, lwd = 1.5)
...
legend("center", title = "Computational method",
       legend = c("MCMC", ## "Normal approximation",
                  "INLA",
                  "Normal approx at MLE"),
       lty = c(1, 2, 3), col = c(1, 4, 2), lwd = 1.5)
@
\caption{Multiple logistic regression BFF analysis of 2992 births with 17
  ...
  Each plot shows the BFF related to the exponential of a regression coefficient
  which can be interpreted as odds ratio. Independent, weakly informative priors
  $\Nor(0, 1/2)$ are assigned to the coefficients under the alternative $H_{1}$,
  and when the coefficient are considered as nuisance parameters under the null
  $H_{0}$. Variables are indicators except early age (0, 20 ,1 15 19, 2 under
  15), gestational age ($0 = \text{no}$, $1 = 36-38$ weeks, $2 = 33-36$ weeks;
  under 33 weeks excluded), isoimmunization ($0 = \text{no}$, $1 = \text{Rh}$,
...
relates to the exponential of a regression coefficient, which can be interpreted
as the increase in odds ratio between neonatal death when increasing the
variable by one unit while keeping all other variables fixed. BFFs were computed
via the Savage-Dickey density ratio where the posterior densities were computed
from 1'000'000 Markov chain Monte Carlo (MCMC) samples (solid black line) as
implemented in Stan \citep{Carpenter2017} or via intergrated nested Laplace
approximation (dashed blue line) as implemented in INLA \citep{Rue2009}. An
improper flat prior was assigned to the intercept under both the null and the
alternative, while independent
$p(\log OR_{i} \mid H_{1}) = \Nor(\log OR_{i} \mid 0, 1/2)$ priors were assigned
to each coefficient under the alternative. This prior represents an alternative
postulating that the median odds ratio is $1$ and that $95\%$ of odds ratios are
in between $1/4$ and $4$ \citep{Greenland2006}. For the analysis of each
coefficient, all other coefficients were considered as nuisance parameters with
the same priors assigned to them under the null as under the alternative. In
addition, the BFF based on a normal approximation to the individual estimates
and their standard errors estimated via maximum likelihood are also shown (red
dotted lines) for comparison. This analysis ignores the multivariate structure
of the data but produces similar results for most variables, while running much
faster than the INLA analysis (taking several second), and which itself runs
much faster than MCMC (taking several minutes).

Due to the sparse nature of the data, most of the BFFs are relatively
uninformative about whether or not the variables exhibit harmful or beneficial
associations with neonatal death. For example, the BFF for the coefficient
`Non-White' (top left panel) is peaked at
$\widehat{\text{OR}}_{\mathrm{ME}} = \Sexpr{round(exp(meesmcmc[coefs == "nonwhite"]), 1)}$
indicating a slightly harmful association between non-white ethnicity and
neonatal death, yet this parameter value receives only anecdotal support over
the alternative ($\kME = \Sexpr{round(kmcmc[coefs == "nonwhite"], 2)}$) and the
corresponding $k = 1$ support interval spans the range from slightly beneficial
up to harmful association.

For most variables, extremely harmful or extremely beneficial associations are
clearly ruled. However, the variables `Gestational age', `Hydramnios', and
`Twin, triplet` are notable exceptions. In each case, the BFFs suggest small up
to very harmful associations with neonatal death. The most extreme among them is
...
yet still unrealistically large. This extreme inflation reflects the fact that
only one death with hydramnios during pregnancy was observed.
...
based on `weakly informative` \citep{Gelman2009} or `default` prior distribution
\citep[e.g., unit-information priors, see][]{Kass1995b} but should acknowledge
this limitation and report sensitivity analyses (e.g., BFFs for different prior
distributions). Another possibility is to base BFF inferences on Bayes factor
bounds, which give a bound on the maximum evidence against parameter values, but
at the cost of losing the ability to quantify evidence \emph{in favour} of
parameter values \citep{Pawel2023}. Finally, where under their control, data
analysts should design experiments and studies so that conclusive inferences can
be drawn from the data collected, and this is also applies to BFF
inferences. % While some research has been done on
% study design with support intervals \citep{Pawel2023}, f
Future research needs to investigate how experiments need to be designed to
enable conclusive inference with BFFs.
