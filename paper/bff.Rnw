...
`$\Pr(\text{the treatment effect is present})$'. One approach to address this, is
...
and denote by $\hat{\psi}_{0}$ maximizer of the log likelihood of the data under
the null and $\hat{\theta}_{1}$, $\hat{\psi}_{1}$ the maximizer under the
alternative hypothesis. Denote by $n V_{0}$ and $n V_{1}$ the modal dispersion
matrices (minus the inverse of the matrix of second-order partial derivatives of
the log likelihood evaluated at the corresponding maximizer). Applying a Laplace
approximation to the logarithm of the BFF \cite[equation 7.27]{OHagan2004} gives
then
% \begin{align*}
...
% (fourth term).
Equation~\eqref{eq:bfapprox} has much intuitive value: if $\theta_{0}$ is
different from $\that_{1}$, the log normalized profile likelihood (first term)
will decrease toward negative infinity, indicating evidence against the
parameter value $\theta_{0}$. On the other hand, the term will be about zero
when $\theta_{0}$ is about $\that_{1}$, so that the sample size $n$ (second
term) increases the log BFF toward positive infinity, indicating evidence for
$\theta_{0}$. The relative accuracy of the priors (third term) and the relative
dispersion (fourth term) lead to further adjustments of the BFF. For instance,
when the observed parameter estimate is likely under the prior, this increases
the evidence for corresponding hypothesis while a misspecified prior that is in
conflict with the parameter estimates lowers the evidence for the corresponding
hypothesis.
...
\citet{Sullivan2012}. This epidemiological study investigated the association
between 14 exposure variables and the risk for neonatal death among pregnant
women. Notably, among the 2992 births only 17 neonatal deaths occurred, leading
to challenges in conducting inferences with so many exposure variables.
...
bglm1 <- brm(death ~ ., data = dat,
             ...
             iter = 1000000, warmup = 1000, cores = 11,
             family = "bernoulli")
...
for (i in seq(1, length(coefs))) {
    ...
    if (coef %in% p1coef) {
        priordens <- dnorm(x = logORseq, mean = pm1, sd = psd1)
    } else if (coef %in% p1coef) {
        priordens <- dnorm(x = logORseq, mean = pm2, sd = psd2)
    } else {
        priordens <- dnorm(x = logORseq, mean = pm3, sd = psd3)
    }
    plot(ORseq, postdens$y/priordens, type = "l", log = "xy", lwd = 1.5,
    ...
    lines(ORseq, inla.dmarginal(x = logORseq, marginal = bglm2$marginals.fixed[[coef]])/priordens,
          lty = 2, col = 4, lwd = 1.5)
...
\caption{Multiple logistic regression BFF analysis of 2992 births with 17
  neonatal deaths from \citet{Neutra1978}. Each plot shows the BFF related to
  the exponential of a regression coefficient which can be interpreted as odds
  ratio. Independent, weakly informative priors $\Nor(0, 1/2)$ are assigned to
  the coefficients under the alternative $H_{1}$, and when the coefficient are
  considered as nuisance parameters under the null $H_{0}$. Variables are
...
as the odds ratio to experience neonatal death when increasing the exposure by
one unit while keeping all other exposures fixed. BFFs were computed via the
Savage-Dickey density ratio and the posterior densities computed via Markov
chain Monte Carlo (solid black line) as implemented in Stan
\citep{Carpenter2017} or via intergrated nested Laplace approximation (dashed
blue line) as implemented in INLA \citep{Rue2009}. An improper flat prior was
assigned to the intercept under both the null and the alternative, while
independent $p(\log OR_{i} \mid H_{1}) = \Nor(\log OR_{i} \mid 0, 1/2)$ priors
were assigned to each coefficient. This prior represents an alternative
...
the same priors assigned to them under the null as under the alternative.

TODO: ADD INTERPRETATION AND DISCUSSION (DATA ARE REALLY UNINFORMATIVE, BFF can
tell that there is absence of evidence for most MEEs, etc.)
