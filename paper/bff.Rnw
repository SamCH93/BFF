...
\author{
 \anonymize{\textbf{Samuel Pawel} \orcidlink{0000-0003-2779-320X}} \\
  ...
  \anonymize{{\color{blue} Preprint version July 2, 2025}}
}
\date{}
...
\citet{Rouder2018} in the context of teaching Bayesian statistics. Another
method called `Bayes factor function' has been proposed by \citet{Johnson2023}.
In this approach, Bayes factors are viewed as a function of a hyperparameter of
the prior \emph{under the alternative hypothesis} but for a fixed null
hypothesis. The Bayes factor function can in some cases be made equivalent to
the support curve if the roles of the null and alternative hypothesis are
reversed and point mass priors are assigned to the parameter under the
alternative, yet this seems rather unnatural from the perspective of both
methods. Finally, a similar attempt of reconciling Bayesian interval estimation
and hypothesis testing with a focus on the `Bayesian evidence value' has been
investigated by \citet{Kelter2022}.

While many elements of support curve inference have been considered in earlier
articles, this paper provides a synthesis of these ideas and considers the
support curve as a counterpart to the \textit{P}-value function. An entirely
novel contribution is the concept of point estimation based on Bayes factors. We
call the resulting estimate the \emph{maximum evidence estimate} (MEE). The MEE
is the parameter value that maximizes the support curve, and as such, is the
...
of a Bayes factor analogue of a \textit{P}-value function, find out how such an
approach can be useful, and how it connects to other approaches. Bayes factors,
just as \textit{P}-values and posterior probabilitites, can be delicate tools
and users should be wary of their limitations, misuses, and misinterpretations
\citep{Tendeiro2019, Wong2022, Tendeiro2024}.
...
due to more intuitive interpretation and computational advantages, we will focus
on nested models only and touch on extensions to non-nested models in
...
evidence of level~$k$ over the specified alternative. A $k$ support set thus
contains all the parameter values that, when considered as individual point null
...
we want to conduct inferences regarding $\theta$. This is a simple but
frequently encountered scenario. For example, suppose we observed a mean test
score of $\bar{y} = \Sexpr{110}$ in a sample of $n = \Sexpr{n}$ students. Also
assume that in the general population the average score is 100 and the standard
deviation is $\kappa = \Sexpr{usd}$. To obtain a Bayes factor for contrasting
...
the specified prior under the alternative, the $k = 1$ support interval spans an
...
@
\caption{Support curve, MEE, and $k = \Sexpr{k}$ support interval for a mean test
  score $\theta$ based on the observed mean $\bar{y} =\Sexpr{round(y, 2)}$ from
  $n = \Sexpr{n}$ observations for different prior distributions for $\theta$
  under the alternative $H_{1}$. A normal likelihood $Y \mid \theta \sim
  \Nor(\theta, \kappa^{2}_{i} = \Sexpr{usd}^2)$ is assumed for the data.}
\label{fig:normal}
\end{figure}
...
parameters under each hypothesis. As the previous example showed, the prior
assigned to the parameters under the alternative hypothesis also has a
substantial impact on SC inferences. Moreover, unlike posterior distributions,
where the influence of the prior diminishes as data accumulate, the sensitivity
of Bayes factors to prior distributions persists even with large samples
\citep{Kass1995}.
...
exists, translating it into a formal prior distribution is often nontrivial. For
...
consistency, without incorporating prior knowledge \citep{Bayarri2012,
  Consonni2018}. Second, prior elicitation techniques formally translate expert
judgments into quantitative priors \citep{OHagan2019, Stefan2022b, Mikkola2024}.
Third, Bayes factor bounds \citep{Berger1987, Sellke2001, Held2018} and
sensitivity analysis \citep{Kass1995, Sinharay2002, Franck2019} explore how
Bayes factors change across a class of priors to assess robustness of
conclusions. Finally, reverse-Bayes analyses identify the priors that lead to
the Bayes factor crossing a tipping point, allowing analysts to assess the
plausibility of such a prior in light of external knowledge
\citep{Good1950,Held2021b}. As all of these techniques are applicable to Bayes
factors, they are also applicable to SCs. We will now focus on more specific
considerations related to SCs.
...
\citep{Robert2014, Wagenmakers2021a, Pawel2023}. Priors for nuisance parameters
may be improper as long as the same prior is assigned under both the null $H_0$
and the alternative $H_1$ so that arbitrary constants cancel out.
...
as such may be preferred in the same situations where default Bayes factors
would be used.

Given these considerations, SCs, just as Bayes factors, are especially useful if
informative priors based prior data or external knowledege are available. If
...
marginalized over the participant probabilities distribution, leading to
\begin{align*}
...
\caption{Support curves with maximum evidence estimate and $k=1$ support
  ...
  multiplying the SC with the prior density.}
...
Standard Model \citep{Afzal2023}. Similarly, our applications of SCs to the coin
...
the interpretation of the SC as quantifying relative predictive accuracy still
holds, yet is more intricate as the comparison does not only involve an
additional parameter but an entirely different model. Moreover, computing the SC
via the Savage-Dickey density ratio is not possible.
...
fact, there is empirical evidence that Bayes factors tend to be misused or
misinterpreted. For example, Bayes factors are sometimes erronously interpreted
as absolute and not relative evidence, or as posterior odds and not as updating
factors \citep{Wong2022, Tendeiro2024}. Bayes factors by construction do not
take into account the prior probabilities of their contrasted hypotheses, so
they may indicate strong support for a hypothesis even though this hypothesis
would still remain unlikely when combined with its prior probability
\citep{Lavine1999, Good2001}. Any type of statistical inference can lead to
distorted scientific inferences if used in a bright-line fashion without
