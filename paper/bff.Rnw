...
similarities and differences of the proposal to conventional frequentist and
Bayesian approaches. We conclude with a discussion of the advantages and
limitations of our framework, and opportunities for future research
...
For one- or two-dimensional focus parameters $\theta$, the BFF can be graphed so
that the relative support for parameter values can be visually assessed. For
higher dimensional focus parameters, this becomes more difficult and the BFF may
need to be summarized in some form.
...
used. For example, based on the conventions from \citet{Jeffreys1961}, a support
...
$k = 1$ support interval spans an wide range, indicating that large
($\theta \approx -0.35$) up to even slightly harmful ($\theta \approx 0.08$)
treatment effects are supported by the data over the alternative.
...
It is of interest to understand the asymptotic behaviour of the BFF. Applying
Laplace's method expanded around the MLE \cite[equation 6]{Kass1995} gives the
following approximation to the logarithm of the BFF
\begin{align*}
...
$i \in \{0, 1\}$. TODO: UNDERSTAND THIS BETTER! WHAT HAPPENS WHEN N GROWS? DOES
...
$\kappa^{2} = \Sexpr{kappa^2}$, and with a local normal prior with unit variance
\citep[a unit-information prior, see][]{Kass1995b} specified under the
alternative. We see that, as the sample size increases, the BFF becomes more
peaked around the true mean, as it should. Similar, the further away the BFF
from the true mean. TODO: IMPROVE THIS DESCRIPTION
...
prior as a consequence of the so-called Savage-Dickey density ratio
\citep{Dickey1971, Verdinelli1995, Wagenmakers2010}. It is, however, important
to emphasize that BFFs based on priors under the alternative that depend on the
null (e.g., commonly used `local' normal or Cauchy priors that are centered
around $\theta_{0}$) cannot be transformed to a genuine posterior distribution
in this way, but multiplication with the prior will result in a different
posterior for every $\theta$.

The link~\eqref{eq:posterior} also provides a convenient way to compute BFFs in
more general settings: One of the many programs for computing Bayesian posterior
distributions, such as Stan \citep{Carpenter2017}, can be used to compute a
...
to another Bayesian inference quantity -- the relative belief ratio
\begin{align}
  \label{eq:RB}
...
factor via the previously mentioned Savage-Dickey density ratio. An estimation
and testing framework centered around the relative belief ratio was developed by
\citet{Evans1997, Evans2015}. This framework is perhaps the closest to the one
presented here. However, as with the posterior distribution there is only a
correspondence between relative belief ratio inferences and BFF inference if the
prior for $\theta$ is the same for every $\theta_{0}$, and it does not hold
anymore when, for example, local priors centered around the value of the null
$\theta_{0}$ are used.
...
Likelihoodists define their support sets based on the relative likelihood
\begin{align*}
...
in favor of the alternative concentrated around $\theta =0.51$. This result
...
TODO: ADD A MORE COMPLICATED EXAMPLE WITH REGRESSION MODELS, FOR EXAMPLE, G
PRIOR, COULD ALSO USE MCMC/INLA TO ESTIMATE POSTERIOR AND THEN USE SAVAGE-DICKEY
TO GET BFF
