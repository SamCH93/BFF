...
The code and data to reproduce our analyses is openly available at XXXX. A
snapshot of the repository at the time of writing is available at XXX. We used
the statistical programming language R version XX \citep{R} for analyses.
