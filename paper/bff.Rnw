...
the parameter under the alternative, which is challenging in itself). However,
the use of Bayes factors comes at the cost of lacking an overarching concept,
such as a \textit{P}-value function or posterior distribution, that can provide
data analyst with a coherent set of point and interval estimates. In practice,
data analysts who wish to perform hypothesis testing with Bayes factors but also
...
$p(y \given H_0)$ by the alternative's marginal likelihood $p(y \given H_1)$
does not change the maximizer of null marginal likelihood $p(y \given H_0)$, the
MEE can be obtained by maximizing $p(y \given H_0)$ without reference to an
...
distribution. Specifically, assuming a `global' and non-degenerate prior under
the alternative, a prior $p(\theta \mid H_{1})$ which does not depend on the
parameter under the null $\theta_{0}$, and also that the priors for the nuisance
parameters satisfy
$p(\psi \mid H_{0}) = p(\psi \mid \theta = \theta_{0}, H_{1})$, we can represent
of the Bayes factor as the ratio of marginal posterior to prior density
evaluated at the tested parameter value \citep[known as Savage-Dickey density
ratio, see e.g.,][]{Dickey1971, Verdinelli1995, Wagenmakers2010}. Hence, the
posterior can be obtained by multiplying the BFF with the prior
\begin{align}
  \label{eq:posterior}
  p(\theta \mid y, H_{1})
...
will result in a different posterior for every $\theta$. Similarly, the
Savage-Dickey density ratio is useless when point priors are assigned to
$\theta$ under the alternative.
...
One of the many programs for computing Bayesian posterior distributions, such as
Stan \citep{Carpenter2017} or INLA \citep{Rue2009}, can be used to compute a
posterior density, which can then be divided by the prior density to obtain a
BFF. The caveat is again that this only works for global priors under the
alternative and with the same prior assigned to the nuisance parameters under
...
quantifies the average true parameter across participants, while the
heterogeneity standard deviation $\tau$ quantifies the heterogeneity of these
true parameters. The Bayes factor for testing
...
Each is supposed to be normally distributed around a common underlying effect
size $\theta$ with (assumed to be known) variance equal to its squared standard
...
the top plot are given by the set of effect size where posterior density is
larger than the prior density. These posteriors represent a synthesis of the
original and replication studies, as they lie somewhere in between the
...
colnames(dat) <- c("death", "nonwhite", "teenages", "nullip", "gestage", "isoimm", "abort", "hydram", "dyslab",
...
bffres <- lapply(X = seq(1, length(coefs)), FUN = function(i) {
    ...
    if (coef == "teenages") {
        ...
        lowerSI <- uniroot(rootFun, lower = log(1/10), upper = meeinla)$root
        upperSI <- uniroot(rootFun, lower = meeinla, upper = log(10))$root
    } else if (coef == "hydram") {
        ## HACK to compute the SI
        lowerSI <- logORseq[which.min(abs(log(bffmcmc[logORseq < meemcmc]) - 0))]
        upperSI <- logORseq[logORseq > meemcmc][
            which.min(abs(log(bffmcmc[logORseq > meemcmc]) - 0))]
    } else {
        lowerSI <- NaN
        upperSI <- NaN
    }
    ...
    out <- list(bffDF,
                rbind(data.frame(coef = names(coefs)[i], lowerSI, upperSI,
                                 mee = meemcmc, k = kmcmc, method = "MCMC"),
                      data.frame(coef = names(coefs)[i], lowerSI, upperSI,
...
to the integrated MLEs. The MCMC analysis took the longest of the three (several
...
and INLA methods (with default settings) can have inaccuracies or cannot be
computed in the tails of the BFF as these represent regions where the posterior
density is nearly zero. Finally, the univariate normal analysis agrees well with
the MCMC and INLA analyses in most cases, with a few exceptions where the BFFs
are skewed (e.g., for the `Hydramnios' variable).
...
harmful associations with neonatal death. The most extreme among them is
`Hydramnios' for which an MEE of
$\widehat{\text{OR}}_{\mathrm{ME}} = \Sexpr{round(exp(subset(meeDF, coef == "Hydramnios" & method == "MCMC")["mee"]), 1)}$
($k = 1$ support interval from
$\Sexpr{round(exp(subset(meeDF, coef == "Hydramnios" & method == "MCMC")["lowerSI"]), 1)}$
to
$\Sexpr{round(exp(subset(meeDF, coef == "Hydramnios" & method == "MCMC")["upperSI"]), 1)}$)
...
$\widehat{\text{OR}}_{\mathrm{ML}} = \Sexpr{round(exp(subset(meeDF, coef == "Hydramnios" & method == "Univariate normal")["mee"]), 1)}$,
but still unrealistically large. This extreme inflation reflects the fact that
only one death was observed with hydramnios during pregnancy. The example
illustrates that just as non-Bayesian methods, BFFs, support intervals, and MEEs
can suffer from small-data artifacts. These could be avoided with a posterior
distribution based on a weakly informative prior that shrinks the posterior
toward more realistic values \citep{Greenland2006}, with the caveat that a
poorly chosen prior may also mask genuine signals from the data.
...
aligns with likelihoodist inference, but includes a natural way to deal with
nuisance parameters.
...
large. Finally, computing BFFs can be challenging. For example, if a BFF is
computed via the Savage-Dickey density ratio from a posterior distribution
computed by MCMC, the BFF may be imprecise at the tails of the posterior, even
with millions of samples. Future work may focus on developing more efficient
techniques for computing BFFs in such settings.
...
were obtained from the \texttt{metadat} R package \citep{White2023}. The code
and data to reproduce our analyses is openly available
