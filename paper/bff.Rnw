...
\author{
  ...
  {\color{blue} Preprint version January 13, 2025}
}
\date{}
...
function of the tested parameter. We then use this \emph{support curve}
...
Approaches related to the support curve have recently been proposed in the
physics community under the names of `Bayes factor surface' \citep{Fowlie2024}
and `\textit{K} ratio' \citep{Afzal2023}. Another method called `Bayes factor
function' has recently been proposed by \citet{Johnson2023}. In this approach,
Bayes factors are viewed as a function of a hyperparameter of the prior
\emph{under the alternative hypothesis} but for a fixed null hypothesis. The
Bayes factor function can in some cases be made equivalent to the support curve
if the roles of the null and alternative hypothesis are reversed and point mass
priors are assigned to the parameter under the alternative, yet this seems
...
a focus on the `Bayesian evidence value' has been recently investigated by
\citet{Kelter2022}.
...
connection to other statistical frameworks (Section~\ref{sec:connections}).
Various real data examples in Section~\ref{sec:applications}, inlcuding
meta-analysis, replication studies, and logistic regression, illustrate
properties of the Bayes factor framework. We conclude with a discussion of
limitations, advantages, and directions for future research
...
becomes more difficult and the SC may need to be summarized in some way, which
...
set (typically an interval) is obtained from `cutting' the SC at $k$ and taking
...
\begin{table}[!htb]
  \centering
  \caption{Support curve $\BF_{01}$, maximum evidence estimate
...
$y = \Sexpr{round(y, 2)}$, the blue SC). The SC has its mode still at the
...
the orange SC with the mean $m = \Sexpr{round(m, 2)}$ based on the eight
previous trials.
...
@
\caption{Support curve, MEE, and $k = \Sexpr{k}$ support interval for a
...
and the alternative, unlike the SC based on the `global' normal prior with
fixed mean $m$. As a result, the interpretation of the SC is different: For
such a `local' normal prior, the SC quantifies the support of parameter values
...
observed data $y$ (as in the case of the orange SC, where the prior was
...
the mean $m$ of the global prior is equal to the data (blue SC), the support
...
alternative has a substantial impact on SC inference. This `sensitivity' of
...
in][]{Kass1995} but focus on specific considerations related to SCs.
...
priors, which do. In the latter case, the interpretation of the SC is more
intricate, since for each parameter value the SC quantifies the support over a
...
visual sensitivity analysis is, for example, to plot different SCs resulting
...
unit-variance and $n$ the sample size. The logarithm of the SC based on a
...
text(x = min(t0seq), y = c(2.5, 1/2.5),
     ...
     pos = 4, cex = 0.75, col = adjustcolor("black", alpha.f = 0.8))
@
\caption{Distribution of the SC for different sample sizes. A data model
  $Y \mid \theta \sim \Nor(\theta, \kappa^{2}/n)$ is assumed and data are
  generated from a true mean $\theta_{*} = \Sexpr{tstar}$ and unit-variance
  $\kappa^{2} = \Sexpr{kappa^2}$. The SC is based on a local normal prior
...
the probability of obtaining a Bayes factor $\BF_{01}$ less than $k < 1$ is at
most $k$ for any prior under the alternative
\begin{align*}
...
from a SC corresponds to a 95\% conservative confidence set and
...
the posterior can be obtained by multiplying the SC with the prior
\begin{align}
  \label{eq:posterior}
  p(\theta \mid y, H_{1})
...
works for global priors under the alternative and with the same prior assigned
to the nuisance parameters under the null and alternative.
...
% write something unique about SC analysis that cannot be done with ordinary
% analysis
...
and the corresponding support curves.
...
\caption{Support curves with maximum evidence estimate and $k=1$ support
  ...
  \Sexpr{round(so, 3)}$ which is used to formulate the prior distribution under
  the alternative $\theta \mid H_{1} \sim \Nor(y_{o}, \sigma^{2}_{o})$. The
  replication effect estimates
  %%  %% were $y_{ri} \in \{\Sexpr{yr}\}$ with standard
  %% %% errors $\sigma_{ri} \in \{\Sexpr{round(sr, 3)}\}$, respectively, and
  %% are assumed to be normally distributed $y_{ri} \mid \theta \sim \Nor(\theta,
  %% \sigma^{2}_{ri})$ and
  are pooled via inverse variance weighting. The posterior is obtained by
  multiplying the BFF with the prior density.}
\label{fig:replication}
\end{figure}


The top plot in Figure~\ref{fig:replication} shows the associated replication
SCs, MEE, and $k=1$ support interval. We see from the support curve based on the
...
original study \Sexpr{yo}. Furthermore, the support curve at the mean difference
of zero indicates strong evidence in favor of no difference over the prior based
...
support curve inferences mostly align with the posterior inferences, that is,
...
replications' support curves and posterior distributions we can see much larger
...
%% The replication from lab 2 produced an effect estimate
...
%% that the SC from lab 1 indicates that there is absence of evidence regarding
%% whether or not the data support the value of no effect ($\BF_{01} \approx 1$ at
%% $\theta = 0$), whereas the SCs from labs 2 and 3 indicate strong and decisive
...
%% SC, which indicates decisive evidence for the MEE at
...
@
%% \caption{Support curves with maximum evidence estimates and $k=1$
...
%% SCs, MEEs, and $k=1$ support intervals. We see that, the SCs peak at the
...
effect of a text-based light projection intervention (``It's illegal to buy
drugs from street dealers'' and a pictogram illustrating the same message) on
the presence of street drug dealers in Amsterdam's red-light district. To this
end, the researchers analyzed 765 one-minute segments of video footage from two
surveillance cameras installed in the red-light district with respect to the
presence/absence of street dealers before and after the intervention, as well as
other covariates (day of the week, camera, number of people, and hours after 8
p.m.). Out of 765 segments, street dealers were identified in 60. This small
number of events makes it challenging to draw conclusive inferences about the
effect of the intervention, as we will see in the following.
...
spans the range from ($\text{OR}=1/\Sexpr{round(1/exp(subset(meeDF,coef== "After intervention" & method == "MCMC")["lowerSI"]), 1)}$) up to hardly any
association ($\text{OR} = 1/\Sexpr{round(1/exp(subset(meeDF, coef== "After intervention" & method == "MCMC")["upperSI"]), 1)}$). This suggests some
negative association between the intervention and the presence of street
dealers, although the extent of the association being relatively unclear.
Moreover, the absence of a control condition makes it difficult to infer whether or
not this association truly represents a causal effect.
...
SC inference requires the formulation of alternative hypotheses. For this
reason, SCs are particularly valuable in contexts where prior data or theories
are available to formulate alternative hypotheses. For example, SCs (under the
...
alternative hypotheses, data analysts may use SCs based on `weakly informative`
...
limitation and report sensitivity analyses (e.g., SCs for different prior
distributions). Another possibility is to base SC inference on Bayes factor
...
\citep{Goodman2016, Greenland2023}. We believe that SCs are useful in this
...
data. We thank Riko Kelter, Andrew Fowlie, Eric-Jan Wagenmakers, František
Bartoš, Leonhard Held, Małgorzata Roos, and Sander Greenland for valuable
comments on drafts of the manuscript. The acknowledgment of these individuals
does not imply their endorsement of the paper.
...
%% at \url{https://doi.org/10.1093/ije/dys213}.
The data from \citet{EjbyeErnst2023} where obtained from the
`\texttt{data.RData}' file available at \url{https://osf.io/qpb47}. The data
...
\url{https://doi.org/10.5281/zenodo.10817311}. We used the statistical
